...
export async function GET(req: NextRequest) {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/api/oncall/schedules`, {
      headers: {
        'Authorization': token,
      },
    });
    ...
    console.error('Error proxying GET /api/oncall/schedules:', error);
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.error('Error proxying POST /api/oncall/schedules:', error);
    return NextResponse.json({ message: 'Internal Server Error' }, { status: 500 });
  }
}
